...
contract Governance {
    ...
    function voting (address _participant, uint _proposal, bool _agree) public {
        ...
        proposes[_proposal].amountVote += uint256(IERC20(govToken).balanceOf(_participant) * 10**18)/IERC20(govToken).totalSupply();
    }
    ...
    function makeCallData () public returns(bytes memory) {
    ...
    function getProposal(uint _idx) public view returns (address, uint, uint, bytes memory, bool, bool, uint) {
